...
void ATcommands::begin(Stream &port) { mySerial = &port; }
...
bool ATcommands::sendCommand(char *command, char *reply, uint16_t timeout) {
  ...
  while (!replyMatch && millis() - timer < timeout) {
    if (mySerial->available()) {
      ...
      idx++;
      // if (strstr(replybuffer, reply) != NULL) replyMatch = true; // Only
      // checks if desired reply is inside replybuffer
      if (strcmp(replybuffer, reply) == 0)
        replyMatch = true;  // This means the reply must start with the desired
